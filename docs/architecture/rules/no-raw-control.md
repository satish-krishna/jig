...
- **It checks that the attribute is present, not that the directive is imported.** This is the widest hole and it is live: a component whose template contains `<button hlmBtn>Save</button>` but whose `imports` array omits `HlmButtonImports` lints clean, builds green, and renders a completely unstyled button. Angular does not error on an unknown attribute applied to a known element, so nothing anywhere in the toolchain objects. The rule teaches "add `hlmBtn`" and then accepts a spelling of it that does nothing at all. Closing it needs a companion rule on the TypeScript side reading the inline `template:` literal against `componentImports` from `tools/lint/ast.ts`; it cannot live here, because `processInlineTemplates` hands the template rules a virtual file with no view of the decorator that would carry the imports.
